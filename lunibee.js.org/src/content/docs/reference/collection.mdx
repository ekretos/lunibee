...
Returns a new Collection with the entries of both collections (`other` wins on key clashes).

### `intersection(other)`

Returns entries whose keys are in both collections.
...
Whether every / at least one of the keys is present.
...
Calls `fn(collection)` and returns the collection unchanged, for side effects in a chain.
