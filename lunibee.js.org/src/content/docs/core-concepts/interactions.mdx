...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandName === "userinfo") {
    ...
    await interaction.reply({
      embeds: [embed],
      ephemeral , // Visible only to the command runner
    });
  }
});
```
...
Besides `isChatInputCommand()`, `isMessageComponent()`, `isModalSubmit()` and
`isAutocomplete()`, component interactions can be narrowed by component type:
`isButton()`, `isStringSelectMenu()`, `isUserSelectMenu()`, `isRoleSelectMenu()`,
`isMentionableSelectMenu()`, `isChannelSelectMenu()` and `isAnySelectMenu()`.
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.customId === "modal_ticket") {
    const subject = interaction.getRequiredInputValue("ticket_subject");
    const description = interaction.getRequiredInputValue("ticket_description");
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (focusedOption.name === "tag") {
    ...
    await interaction.respond(
      filtered.map(tag => ({ name , value: tag }))
    );
  }
});
```
