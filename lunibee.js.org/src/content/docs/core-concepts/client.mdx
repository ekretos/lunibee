...
### Using `IntentsBitField`

A discord.js-style bitfield, accepted anywhere intents are:

```ts
import { Client, IntentsBitField } from "lunibee";
...
intents.has("Guilds");  // true
intents.toArray();      // ["Guilds", "GuildMessages", "MessageContent"]
