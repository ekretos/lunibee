  ...
  public addOptions(...options: APISelectOption[]): this {
    ...
    for (const option of options) {
      ...
      if (option.description)
        validateText(option.description, 100, "Select option description");
    }
    this.#data.options = [
      ...current,
...
export class ButtonBuilder {
  ...
  public setURL(value: string): this {
    ...
    try {
      ...
      throw new TypeError("Button URL must be a valid URL.", { cause: error });
    }
...
export class ModalBuilder {
  ...
  public addComponents(
    ...
    if (this.#components.length + components.length > 5)
      throw new RangeError("A modal cannot contain more than 5 action rows.");
    this.#components.push(...components);
    return this;
  }
...
function validateLength(value: number, field: string, max = 4000): void {
  if (!Number.isInteger(value) || value < 0 || value > max)
    throw new RangeError(`${field} must be an integer between 0 and ${max}.`);
}
