...
export class BaseOptionBuilder {
    ...
    /** Creates an option builder with its Discord type. */ public constructor(type: number) { this.data = { type }; }
    /** Sets the option name. */ public setName(value: string): this { validateName(value, "Option name"); this.data.name = value; return this; }
    /** Sets the option description. */ public setDescription(value: string): this { validateText(value, "Option description", 100); this.data.description = value; return this; }
    /** Sets whether the option is required. */ public setRequired(value = true): this { this.data.required = value; return this; }
    /** Enables or disables autocomplete. */ public setAutocomplete(value = true): this { this.data.autocomplete = value; return this; }
    /** Serializes the option. */ public toJSON(): Record<string, unknown> { validateOption(this.data); return structuredClone(this.data); }
}
...
export class IntegerOptionBuilder extends NumberOptionBuilder { /** Creates an integer option. */ public constructor() { super(); this.data.type = 4; } }
...
function validateOption(data: Record<string, unknown>): void { if (typeof data.name !== "string" || !data.name) throw new RangeError("Option name is required."); if (typeof data.description !== "string" || !data.description) throw new RangeError("Option description is required."); if (data.autocomplete === true && data.choices) throw new RangeError("Autocomplete options cannot define choices."); }
