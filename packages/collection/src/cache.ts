...
export class Cache<K, V> {
    ...
    public constructor(options: CacheOptions = {}) {
        ...
        if (this.#ttl > 0) {
            ...
            if (!Number.isFinite(interval) || interval < 1) throw new RangeError("Cache sweepInterval must be positive.");
    ...
    public has(key: K): boolean { return this.get(key) !== undefined; }
    /** Stores an entry and evicts the oldest entries when the bound is exceeded. */
    public set(key: K, value: V): this { const expiresAt = this.#ttl > 0 ? Date.now() + this.#ttl : 0; this.#entries.delete(key); this.#entries.set(key, { value, expiresAt }); while (this.#entries.size > this.#maxSize) { const oldest = this.#entries.firstKey(); if (oldest === undefined) break; this.#entries.delete(oldest); } return this; }
    /** Invalidates one key. */
    ...
    public invalidate(predicate: (value: V, key: K) => boolean): number { return this.#entries.sweep((entry, key) => predicate(entry.value, key)); }
    /** Removes expired entries and returns the number removed. */
    public sweep(): number { return this.#entries.sweep(entry => this.#expired(entry)); }
