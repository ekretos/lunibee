...
export class ShardBus {
    ...
    #counter = 0;
    /** Creates a bus scoped to a shard and an application namespace. */
    public constructor(shardId: number, channelName: string) {
        ...
        if (!channelName?.trim()) throw new TypeError("An application-specific shard channel name is required.");
        this.#shardId = shardId;
        this.#namespace = channelName.trim();
        this.channelName = `lunibee:${this.#namespace}`;
        this.#channel = new BroadcastChannel(this.channelName);
        this.#channel.addEventListener("message", event => this.#dispatch(event.data as ShardMessage));
    }
    /** Registers a handler for one message type. */ public on<T>(type: string, handler: ShardMessageHandler<T>): this { if (!type.trim() || typeof handler !== "function") throw new TypeError("Shard message type and handler are required."); let handlers = this.#handlers.get(type); if (!handlers) this.#handlers.set(type, handlers = new Set()); handlers.add(handler as ShardMessageHandler); return this; }
    /** Removes a message handler. */ public off<T>(type: string, handler: ShardMessageHandler<T>): this { this.#handlers.get(type)?.delete(handler as ShardMessageHandler); return this; }
    /** Sends a message to one shard. */ public send<T>(target: number, type: string, data: T): string { if (!Number.isInteger(target) || target < 0) throw new RangeError("Shard target must be a non-negative integer."); return this.#publish(target, type, data); }
    /** Broadcasts a message to every shard except the sender. */ public broadcast<T>(type: string, data: T): string { return this.#publish(null, type, data); }
    /** Closes the transport and releases the channel. */ public close(): void { this.#channel.close(); this.#handlers.clear(); }
    #publish<T>(target: number | null, type: string, data: T): string { if (!type.trim()) throw new TypeError("Shard message type is required."); const id = `${this.#namespace}:${this.#shardId}:${++this.#counter}`; this.#channel.postMessage({ source: this.#shardId, target, type, data, id } satisfies ShardMessage<T>); return id; }
