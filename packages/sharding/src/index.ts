...
export class ShardManager {
    ...
    /** Immutable shard manager configuration. */ readonly #options: ShardManagerOptions;
    /** Whether shard initialization has completed. */ #resolved = false;
    ...
    public constructor(options: ShardManagerOptions) { if (!options.token?.trim()) throw new TypeError("A Discord bot token is required."); if (!Number.isInteger(options.intents) || options.intents < 0) throw new TypeError("Gateway intents must be a non-negative integer."); this.#options = { ...options }; this.shardCount = options.shardCount === "auto" || options.shardCount === undefined ? 1 : options.shardCount; if (!Number.isInteger(this.shardCount) || this.shardCount < 1) throw new RangeError("Shard count must be a positive integer."); if (options.shardCount !== "auto") this.#initialize(this.shardCount); }
    /** Retrieves Discord's recommended shard count. @returns Recommended shard count. @throws {Error} If the discovery request fails or returns invalid data. */
    public async fetchRecommendedShardCount(): Promise<number> { const response = await fetch("https://discord.com/api/v10/gateway/bot", { headers: { Authorization: `Bot ${this.#options.token}`, "User-Agent": "Lunibee/0.1.0" } }); if (!response.ok) throw new Error(`Gateway discovery failed with status ${response.status}`); const data = await response.json() as { shards?: unknown }; const shardCount = data.shards; if (typeof shardCount !== "number" || !Number.isInteger(shardCount) || shardCount < 1) throw new Error("Gateway discovery returned an invalid shard count."); return shardCount; }
    /** Connects all shards sequentially, resolving `auto` shard count before any Gateway is constructed. @returns A promise fulfilled after all shards connect. @throws {Error} If any shard fails to connect. */
    ...
    /** Closes every shard and releases Gateway resources. @returns Nothing. */ public destroy(): void { for (const shard of this.shards.values()) shard.close(); }
    ...
    /** Resolves lazy shard initialization. @returns A promise fulfilled after initialization. */
