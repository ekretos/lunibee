...
export interface ShardManagerOptions {
    ...
    reconnect?: boolean;
    /** Delay between shard starts in milliseconds. */
    spawnDelay?: number;
}
...
export class ShardManager {
    ...
    public constructor(options: ShardManagerOptions) {
        ...
        for (let id = 0; id < this.shardCount; id++) {
            this.shards.set(id, this.#createShard(id));
        }
    }
    ...
    public async fetchRecommendedShardCount(): Promise<number> {
        ...
        if (!Number.isInteger(data.shards) || data.shards < 1) {
            throw new Error("Gateway discovery returned an invalid shard count.");
        }
        return data.shards;
    }

    /** Connects all shards sequentially to avoid an avoidable connection burst. */
    public async connect(): Promise<void> {
        for (const [id, shard] of this.shards) {
            try {
                await shard.connect();
            } catch (error) {
                throw new Error(`Failed to connect shard ${id}.`, { cause: error });
            }

            if (id + 1 < this.shardCount && this.#options.spawnDelay && this.#options.spawnDelay > 0) {
                await Bun.sleep(this.#options.spawnDelay);
            }
        }
    }

    /** Closes every shard and releases its Gateway resources. */
    public destroy(): void {
