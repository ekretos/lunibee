    ...
    try {
      ...
      this.#emit(
        ...
        error instanceof VoiceError
          ? error
          : new VoiceError("Voice gateway cleanup failed.", { cause: error }),
      );
    }
    try {
      this.udp?.close();
      ...
      this.#emit(
        ...
        error instanceof VoiceError
          ? error
          : new VoiceError("Voice UDP cleanup failed.", { cause: error }),
      );
    }
    ...
    for (const listener of this.#listeners.get("stateChange") ?? []) {
      try {
        ...
        this.#emit(
          ...
          error instanceof VoiceError
            ? error
            : new VoiceError("Voice event listener failed.", { cause: error }),
        );
      }
