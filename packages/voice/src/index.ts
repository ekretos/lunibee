...
export class VoiceError extends Error { /** Creates a voice error. */ public constructor(message: string, options?: ErrorOptions) { super(message, options); this.name = "VoiceError"; } }
...
export class VoiceConnection {
    ...
    /** Registers a voice lifecycle listener. */ public on<K extends keyof VoiceEvents>(event: K, listener: (...args: VoiceEvents[K]) => unknown): this { if (typeof listener !== "function") throw new TypeError("Voice listener must be a function."); let listeners = this.#listeners.get(event); if (!listeners) this.#listeners.set(event, listeners = new Set()); listeners.add(listener); return this; }
    /** Removes a voice lifecycle listener. */ public off<K extends keyof VoiceEvents>(event: K, listener: (...args: VoiceEvents[K]) => unknown): this { this.#listeners.get(event)?.delete(listener); return this; }
    /** Attaches protocol transports, replacing and cleaning up the old pair when present. */
    public attachTransports(gateway: VoiceGatewayTransport, udp: VoiceUdpTransport): this { this.assertUsable(); if (!gateway || !udp) throw new TypeError("Both voice gateway and UDP transports are required."); if (this.gateway || this.udp) this.#closeTransports(); this.gateway = gateway; this.udp = udp; return this; }
    /** Begins the connection lifecycle. */ public connect(): void { this.assertUsable(); if (this.state === "connected") return; this.#transition("connecting"); if (this.state !== "connecting") return; this.#transition("connected"); }
    /** Disconnects the current voice session. */ public disconnect(): void { if (this.state === "destroyed") return; this.#closeTransports(); this.#transition("disconnected"); }
    /** Permanently destroys this voice connection. */ public destroy(): void { if (this.state === "destroyed") return; this.#closeTransports(); this.channelId = undefined; this.#transition("destroyed"); this.#listeners.clear(); }
    /** Updates the voice channel and connection state. */ public setChannel(channelId: string): void { this.assertUsable(); if (!channelId.trim()) throw new TypeError("A voice channel ID is required."); this.channelId = channelId; }
    /** Updates self mute/deaf state. */ public setSuppression(options: { selfMute?: boolean; selfDeaf?: boolean }): void { this.assertUsable(); if (options.selfMute !== undefined) this.selfMute = options.selfMute; if (options.selfDeaf !== undefined) this.selfDeaf = options.selfDeaf; }
    /** Publishes a speaking state through the attached voice gateway. */ public setSpeaking(flags = SpeakingFlags.Microphone): void { this.assertUsable(); if (!this.gateway) throw new VoiceError("A voice gateway transport is required to change speaking state."); this.gateway.send({ op: 5, d: { speaking: flags, delay: 0, ssrc: 0 } }); this.#emit("speaking", flags, 0); }
    private assertUsable(): void { if (this.state === "destroyed") throw new VoiceError("Voice connection has been destroyed."); }
    private #closeTransports(): void { try { this.gateway?.close(1000, "Voice connection replaced or closed"); } catch (error) { this.#emit("error", error instanceof VoiceError ? error : new VoiceError("Voice gateway cleanup failed.", { cause: error })); } try { this.udp?.close(); } catch (error) { this.#emit("error", error instanceof VoiceError ? error : new VoiceError("Voice UDP cleanup failed.", { cause: error })); } this.gateway = undefined; this.udp = undefined; }
