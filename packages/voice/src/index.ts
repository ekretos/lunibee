...
export class VoiceConnection {
    ...
    public constructor(options: string | VoiceConnectionOptions) {
        ...
        if (!resolved.guildId?.trim()) {
            throw new TypeError("A guild ID is required for a voice connection.");
        }
    ...
    public on<K extends keyof VoiceEvents>(event: K, listener: (...args: VoiceEvents[K]) => unknown): this {
    ...
    public off<K extends keyof VoiceEvents>(event: K, listener: (...args: VoiceEvents[K]) => unknown): this {
    ...
    public attachTransports(gateway: VoiceGatewayTransport, udp: VoiceUdpTransport): this {
        this.assertUsable();
        if (!gateway || !udp) throw new TypeError("Both voice gateway and UDP transports are required.");
    ...
    public setSpeaking(flags = SpeakingFlags.Microphone): void {
        this.assertUsable();
        if (!this.gateway) {
            throw new VoiceError("A voice gateway transport is required to change speaking state.");
        }
    ...
    #closeTransports(): void {
        try {
            ...
            this.#emit(
                "error",
                error instanceof VoiceError ? error : new VoiceError("Voice gateway cleanup failed.", { cause: error }),
            );
        }
        try {
            this.udp?.close();
            ...
            this.#emit(
                "error",
                error instanceof VoiceError ? error : new VoiceError("Voice UDP cleanup failed.", { cause: error }),
            );
        }
    ...
    #transition(next: VoiceConnectionState): void {
        ...
        for (const listener of this.#listeners.get("stateChange") ?? []) {
            try {
                ...
                this.#emit(
                    "error",
                    error instanceof VoiceError ? error : new VoiceError("Voice event listener failed.", { cause: error }),
                );
            }
            if (this.state !== next && next !== VoiceConnectionState.Destroyed) return;
        }
    }

    /** Emits a voice event while isolating consumer failures. @param event Event name. @param args Event arguments. @returns Nothing. */
    #emit<K extends keyof VoiceEvents>(event: K, ...args: VoiceEvents[K]): void {
