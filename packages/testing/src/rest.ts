...
export interface RecordedRequest { /** HTTP method. */ method: string; /** API path. */ path: string; /** Request body. */ body?: unknown; }
...
export class MockREST extends REST {
    ...
    public constructor(responses: Record<string, unknown> = {}, retryPolicy?: RetryPolicy) { super({ baseURL: "https://test.invalid", retryPolicy }); for (const [path, response] of Object.entries(responses)) this.#responses.set(path, response); }
    ...
    public override async request<T>(method: string, path: string, body?: unknown): Promise<T> { this.requests.push({ method, path, body }); if (!this.#responses.has(path)) throw new Error(`No mock response configured for ${path}`); return this.#responses.get(path) as T; }
}
