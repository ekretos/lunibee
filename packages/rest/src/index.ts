...
export interface RESTRequestOptions { /** Abort signal used to cancel the request and any queued wait. */ signal?: AbortSignal; }

/** Configures which REST failures may be retried. */
export interface RetryPolicy { /** Maximum retry attempts. */ maxRetries: number; /** Determines whether a request may be retried. @param method HTTP method. @param status HTTP status, or zero for transport errors. @returns True when retrying is safe under the policy. */ shouldRetry(method: string, status: number): boolean; /** Calculates delay before the next attempt. @param attempt Zero-based retry number. @param retryAfter Server-provided delay, when available. @returns Delay in milliseconds. */ getDelay(attempt: number, retryAfter?: number): number; }
...
export function createRetryPolicy(maxRetries = 2): RetryPolicy { if (!Number.isFinite(maxRetries) || maxRetries < 0) throw new TypeError("maxRetries must be a non-negative finite number."); return { maxRetries: Math.floor(maxRetries), shouldRetry(method, status) { const normalized = method.toUpperCase(); return status === 429 || (status >= 500 && status <= 599 && ["GET", "HEAD", "PUT", "DELETE"].includes(normalized)); }, getDelay(attempt, retryAfter) { if (retryAfter !== undefined) return Math.max(0, retryAfter * 1000); return Math.min(10_000, 500 * 2 ** attempt) + Math.random() * 250; } }; }

/** Bun-native REST transport with Discord bucket-aware rate limiting, retries, and cancellation. */
export class REST {
    readonly #baseURL: string; readonly #timeout: number; readonly #retryPolicy: RetryPolicy; #token?: string; #globalResetAt = 0;
    readonly #buckets = new Map<string, Bucket>(); readonly #routeBuckets = new Map<string, string>();
    /** Creates a REST transport. @param options Transport configuration. @throws {TypeError} If retry configuration is invalid. */
    public constructor(options: { token?: string; timeout?: number; retries?: number; retryPolicy?: RetryPolicy; baseURL?: string } = {}) { this.#token = options.token; this.#timeout = Math.max(1, options.timeout ?? 15_000); this.#retryPolicy = options.retryPolicy ?? createRetryPolicy(options.retries ?? 2); this.#baseURL = (options.baseURL ?? "https://discord.com/api/v10").replace(/\/$/, ""); }
    ...
    public async request<T>(method: string, path: string, body?: unknown, options: RESTRequestOptions = {}): Promise<T> {
        ...
        let bucket = this.#buckets.get(bucketKey) ?? { remaining: 1, resetAt: 0, queue: Promise.resolve() };
        ...
        try {
            for (let attempt = 0; attempt <= this.#retryPolicy.maxRetries; attempt++) {
                ...
                if (options.signal?.aborted) throw this.#abortError(path, options.signal.reason);
                options.signal?.addEventListener("abort", onAbort, { once: true });
                const timer = setTimeout(() => controller.abort(new DOMException("REST request timeout", "TimeoutError")), this.#timeout);
                try {
                    ...
                    const retryAfter = response.status === 429 ? this.#retryAfter(response, data) : undefined;
                    if (data.global && retryAfter !== undefined) this.#globalResetAt = Date.now() + retryAfter * 1000;
                    if (attempt < this.#retryPolicy.maxRetries && this.#retryPolicy.shouldRetry(normalizedMethod, response.status)) {
                        await this.#sleep(this.#retryPolicy.getDelay(attempt, retryAfter), options.signal, path);
                        continue;
                    }
                    ...
                    if (!isTimeout && attempt < this.#retryPolicy.maxRetries && this.#retryPolicy.shouldRetry(normalizedMethod, 0)) { await this.#sleep(this.#retryPolicy.getDelay(attempt), options.signal, path); continue; }
                    throw new RESTError(isTimeout ? `Discord REST request timed out after ${this.#timeout}ms` : "Discord REST request failed", 0, undefined, undefined, { method: normalizedMethod, path, cause: error });
    ...
    /** Sends a GET request. @param path API path. @param options Cancellation options. @returns Decoded response. @throws {RESTError} When the request fails. */ public get<T>(path: string, options?: RESTRequestOptions): Promise<T> { return this.request<T>("GET", path, undefined, options); }
    /** Sends a POST request. @param path API path. @param body Optional JSON body. @param options Cancellation options. @returns Decoded response. @throws {RESTError} When the request fails. */ public post<T>(path: string, body?: unknown, options?: RESTRequestOptions): Promise<T> { return this.request<T>("POST", path, body, options); }
    /** Sends a PATCH request. @param path API path. @param body Optional JSON body. @param options Cancellation options. @returns Decoded response. @throws {RESTError} When the request fails. */ public patch<T>(path: string, body?: unknown, options?: RESTRequestOptions): Promise<T> { return this.request<T>("PATCH", path, body, options); }
    /** Sends a PUT request. @param path API path. @param body Optional JSON body. @param options Cancellation options. @returns Decoded response. @throws {RESTError} When the request fails. */ public put<T>(path: string, body?: unknown, options?: RESTRequestOptions): Promise<T> { return this.request<T>("PUT", path, body, options); }
    /** Sends a DELETE request. @param path API path. @param options Cancellation options. @returns Decoded response. @throws {RESTError} When the request fails. */ public delete<T>(path: string, options?: RESTRequestOptions): Promise<T> { return this.request<T>("DELETE", path, undefined, options); }
    ...
    async #wait(bucket: Bucket, signal: AbortSignal | undefined, path: string): Promise<void> { const delay = bucket.resetAt - Date.now(); if (bucket.remaining <= 0 && delay > 0) await this.#sleep(delay, signal, path); }
    ...
    async #waitGlobal(signal: AbortSignal | undefined, path: string): Promise<void> { const delay = this.#globalResetAt - Date.now(); if (delay > 0) await this.#sleep(delay, signal, path); }
    ...
    async #sleep(delay: number, signal: AbortSignal | undefined, path: string): Promise<void> { if (delay <= 0) return; await new Promise<void>((resolve, reject) => { const timer = setTimeout(resolve, delay); const abort = () => { clearTimeout(timer); reject(this.#abortError(path, signal?.reason)); }; if (signal?.aborted) return abort(); signal?.addEventListener("abort", abort, { once: true }); }); }
    ...
    async #abortable(promise: Promise<void>, signal: AbortSignal | undefined, path: string): Promise<void> { if (!signal) return promise; if (signal.aborted) throw this.#abortError(path, signal.reason); return Promise.race([promise, new Promise<void>((_, reject) => signal.addEventListener("abort", () => reject(this.#abortError(path, signal.reason)), { once: true }))]); }
    ...
    #abortError(path: string, reason: unknown): RESTError { return new RESTError(reason instanceof Error ? reason.message : "Discord REST request was aborted", 0, undefined, undefined, { path, cause: reason }); }
    ...
    #update(response: Response, bucket: Bucket, route: string, bucketKey: string): Bucket { const serverBucket = response.headers.get("X-RateLimit-Bucket"); if (serverBucket) { this.#routeBuckets.set(route, serverBucket); if (serverBucket !== bucketKey) { const shared = this.#buckets.get(serverBucket) ?? bucket; this.#buckets.set(serverBucket, shared); bucket = shared; } } const remaining = Number(response.headers.get("X-RateLimit-Remaining")); const resetAfter = Number(response.headers.get("X-RateLimit-Reset-After")); const reset = Number(response.headers.get("X-RateLimit-Reset")); if (Number.isFinite(remaining)) bucket.remaining = Math.max(0, remaining); if (Number.isFinite(resetAfter)) bucket.resetAt = Date.now() + Math.max(0, resetAfter) * 1000; else if (Number.isFinite(reset)) bucket.resetAt = reset * 1000; if (response.status === 429) { const retryAfter = Number(response.headers.get("Retry-After")); if (Number.isFinite(retryAfter)) bucket.resetAt = Math.max(bucket.resetAt, Date.now() + retryAfter * 1000); } if (response.headers.get("X-RateLimit-Global") === "true") { const retryAfter = Number(response.headers.get("Retry-After")); if (Number.isFinite(retryAfter)) this.#globalResetAt = Math.max(this.#globalResetAt, Date.now() + retryAfter * 1000); } return bucket; }
}
