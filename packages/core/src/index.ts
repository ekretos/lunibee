...
export interface ClientEvents {
    /** Fired after the Gateway sends READY. */
    ready: [ClientUser];
    /** Fired for every raw Gateway dispatch. */
    raw: [unknown];
    /** Fired when the Gateway reports an error. */
    error: [Error];
    /** Fired when Discord creates a message. */
    messageCreate: [Message];
    /** Fired when Discord updates a message. */
    messageUpdate: [Message];
    /** Fired when Discord deletes a message. */
 ...
 *
 * Listener failures are isolated so one consumer cannot break the Gateway
 * dispatch loop. Both synchronous throws and asynchronous rejections are
 * forwarded to the client's error event.
 */
class EventEmitter<Events extends Record<string, unknown[]>> {
    ...
    /** Emits an event while protecting the dispatch loop from listener failures. */
    protected emit<K extends keyof Events>(event: K, ...args: Events[K]): void {
        for (const listener of this.#listeners.get(event) ?? []) {
            try {
                const result = listener(...args);
                if (result && typeof (result as PromiseLike<unknown>).then === "function") {
                    void Promise.resolve(result).catch(error => this.handleListenerError(event, error));
                }
            } catch (error) {
    ...
    /**
     * Handles a listener failure without recursively dispatching an error
     * event when the failing listener already belongs to that event.
     */
    private handleListenerError(event: keyof Events, error: unknown): void {
        if (event === "error") return;

        const normalized = error instanceof Error ? error : new Error(String(error));

        for (const listener of this.#listeners.get("error" as keyof Events) ?? []) {
            try {
                ...
                // Error handlers must never take down the dispatch loop.
            }
        }
    }
}
...
export class Client extends EventEmitter<ClientEvents> implements InteractionClient {
    ...
    public editInteractionReply(
        ...
        return this.rest.patch(
            `/webhooks/${this.user?.id ?? "@me"}/${token}/messages/@original`,
            data
        );
    }
