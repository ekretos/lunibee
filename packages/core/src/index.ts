...
class EventEmitter<Events extends Record<string, unknown[]>> {
    ...
    protected emit<K extends keyof Events>(event: K, ...args: Events[K]): boolean {
        ...
        for (const listener of [...listeners]) {
            try {
                ...
                if (result && typeof (result as PromiseLike<unknown>).then === "function") void Promise.resolve(result).catch(error => this.#handleError(event, error));
    ...
    #handleError(event: keyof Events, error: unknown): void {
        ...
        for (const listener of [...(this.#listeners.get("error" as keyof Events) ?? [])]) {
            try { void listener(normalized); } catch { /* Error handlers are intentionally isolated. */ }
        }
    }
}
