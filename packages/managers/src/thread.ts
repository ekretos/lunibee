...
export class ThreadManager {
    ...
    public async createFromMessage(messageId: string, options: { name: string; autoArchiveDuration?: 60 | 1440 | 4320 | 10080; rateLimitPerUser?: number }): Promise<Channel> { const data = await this.#rest.post<ConstructorParameters<typeof Channel>[0]>(Routes.messageThread(this.#channelId, messageId), { name: options.name, auto_archive_duration: options.autoArchiveDuration, rate_limit_per_user: options.rateLimitPerUser }); return new Channel(data, this.#context); }
}
