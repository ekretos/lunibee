...
export class MessageManager {
    ...
    public async fetch(messageId: string): Promise<Message> { const data = await this.#rest.get<ConstructorParameters<typeof Message>[0] | ConstructorParameters<typeof Message>[0][]>(Routes.message(this.#channelId, messageId)); const message = Array.isArray(data) ? data[0] : data; if (!message) throw new Error("Discord returned no message payload."); return this.upsert(message); }
    /** Fetches several messages by ID. @param messageIds Message identifiers. @returns Canonical messages. */
    public fetchMany(messageIds: Iterable<string>): Promise<Message[]> { return Promise.all(Array.from(messageIds, messageId => this.fetch(messageId))); }
