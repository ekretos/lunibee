...
import { Channel, Message, User } from "@lunibee/structures";
...
export class ChannelManager extends Manager<string, Channel> {
    ...
    public async fetchMessages(channelId: string, options: MessageFetchOptions = {}): Promise<Message[]> {
        ...
        if (options.around !== undefined) query.set("around", options.around);
        if (options.limit !== undefined) query.set("limit", String(options.limit));
        const path = `${Routes.channelMessages(channelId)}${query.size ? `?${query}` : ""}`;
        const data = await this.#rest.get<ConstructorParameters<typeof Message>[0][]>(path);
    ...
    public async fetchReactions(channelId: string, messageId: string, emoji: string, options: ReactionFetchOptions = {}): Promise<User[]> {
        ...
        if (options.after !== undefined) query.set("after", options.after);
        if (options.limit !== undefined) query.set("limit", String(options.limit));
        const path = `${Routes.messageReactions(channelId, messageId, emoji)}${query.size ? `?${query}` : ""}`;
