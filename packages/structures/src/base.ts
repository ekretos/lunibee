...
export class BaseStructure {
    ...
    public constructor(id: string) {
        if (!/^\d{1,20}$/.test(id))
            throw new TypeError(
                "A Discord structure requires a valid snowflake ID.",
            );
        this.id = id;
    }
    ...
    public constructor(
        ...
        if (!Number.isInteger(data.type) || data.type < 0)
            throw new RangeError(
                "A Discord channel requires a valid channel type.",
            );
        this.type = data.type;
        this.name = data.name;
...
export class Guild extends BaseStructure {
    ...
    /** ID of the rules channel for Community guilds. */
    ...
    /** Maximum number of members the guild can have. */
    public maxMembers: number;
    /** Required MFA level for administrators. */
    ...
    public discoverySplashURL(options: ImageURLOptions = {}): string | null {
        ...
        return cdnURL(
            `/discovery-splashes/${this.id}`,
            this.discoverySplash,
            options,
        );
    }

    /** Returns a widget image URL for the guild.
     * @param style Widget style: shield (default), banner1, banner2, banner3, banner4.
     */
    public widgetImageURL(
        style:
            "shield" | "banner1" | "banner2" | "banner3" | "banner4" = "shield",
    ): string {
    ...
    /** Whether the guild is an official Discord Partner. */
    public get isPartnered(): boolean {
        return this.hasFeature("PARTNERED");
    }

    /** Whether the guild is verified by Discord. */
    public get isVerified(): boolean {
        return this.hasFeature("VERIFIED");
    }

    /** Whether the guild has any active boost subscriptions. */
    public get isBoosted(): boolean {
        return this.premiumSubscriptionCount > 0;
    }

    /** Alias for premiumTier — the guild's current Nitro boost level (0–3). */
