...
export class Gateway {
  ...
  #options: Required<Omit<GatewayOptions, "properties" | "presence">> & {
  ...
  public send(payload: GatewayPayload): boolean {
    if (!payload || !Number.isInteger(payload.op))
      throw new TypeError("Gateway payload must contain an integer opcode.");
  ...
  #close(ws: WebSocket, code: number): void {
    ...
    if (this.#closed || !this.#options.reconnect || action === "stop") {
      this.#setState(this.#closed ? GatewayState.Closed : GatewayState.Connect);
      if (this.#connectPromise)
        this.#settleConnect(
          new GatewayError(`Gateway closed before READY (code ${code}).`, code),
        );
      return;
    }
    if (action === "identify") {
  ...
  #scheduleReconnect(): void {
    ...
    this.#reconnectTimer = setTimeout(() => {
      ...
      this.#open(
        this.#resumeURL ?? "wss://gateway.discord.gg/?v=10&encoding=json",
      );
    }, delay + jitter);
  }
