...
export class HandlerRegistry<Events extends Record<string, unknown[]>> {
    ...
    public on<K extends keyof Events>(event: K, handler: (...args: Events[K]) => unknown): this { let handlers = this.#handlers.get(event); if (!handlers) { handlers = new Set(); this.#handlers.set(event, handlers); } handlers.add(handler); return this; }
    /** Registers a one-shot handler. @param event Event key. @param handler Handler callback. @returns This registry. */
    public once<K extends keyof Events>(event: K, handler: (...args: Events[K]) => unknown): this { const wrapped = (...args: Events[K]) => { this.off(event, wrapped); return handler(...args); }; return this.on(event, wrapped); }
    /** Removes a handler. @param event Event key. @param handler Handler callback. @returns This registry. */
    public off<K extends keyof Events>(event: K, handler: (...args: Events[K]) => unknown): this { this.#handlers.get(event)?.delete(handler); return this; }
    ...
    public async dispatch<K extends keyof Events>(event: K, ...args: Events[K]): Promise<void> { for (const handler of this.#handlers.get(event) ?? []) await handler(...args); }
    ...
    public emit<K extends keyof Events>(event: K, ...args: Events[K]): Promise<void> { return this.dispatch(event, ...args); }
}