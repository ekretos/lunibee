...
describe("Client Events & Dispatch Full Coverage", () => {
  test("Client handles Gateway lifecycle and dispatch events", async () => {
    ...
    client.on("messageDeleteBulk", () => received.push("messageDeleteBulk"));
    ...
    client.on("interactionCreate", () => received.push("interactionCreate"));

    const gw = client.ws;

    gw.emit("READY", { user: { id: "100", username: "BotUser" } });
    expect(client.isReady()).toBe(true);
    expect(typeof client.uptime === "number" || client.uptime === null).toBe(
      true,
    );
    ...
    gw.emit("CHANNEL_UPDATE", { id: "700", name: "renamed-channel", type: 0 });
    gw.emit("CHANNEL_DELETE", { id: "700", name: "renamed-channel", type: 0 });
