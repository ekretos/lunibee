...
test("ChannelManager.sendMessage posts a message and returns Message", async () => {
    ...
    const result = await channels.sendMessage(message.channel_id, { content: "hello" });
...
test("ChannelManager.sendMessage propagates REST errors", async () => {
    ...
    const rest = { post: async () => { throw error; } };
    const channels = new ChannelManager(rest as never);

    await expect(channels.sendMessage("123456789012345680", { content: "hello" })).rejects.toBe(error);
});
...
test("ChannelManager.sendMessage validates channel IDs through Routes", async () => {
    ...
    expect(channels.sendMessage("not-a-snowflake", { content: "hello" })).rejects.toThrow(TypeError);
});
