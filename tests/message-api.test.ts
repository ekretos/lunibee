...
function manager() {
  ...
  const rest = {
    ...
    post: async <T>(path: string, body?: unknown): Promise<T> => {
      ...
      return (
        path.endsWith("/messages") || path.includes("/crosspost")
          ? message
          : thread
      ) as T;
    },
...
test("fetches messages with query parameters", async () => {
  ...
  expect(calls[0]?.path).toBe(
    `/channels/${channelId}/messages?before=${messageId}&limit=50`,
  );
});
...
test("supports pins and message threads", async () => {
  ...
  const result = await channels.createThreadFromMessage(channelId, messageId, {
    name: "discussion",
  });
...
test("message routes preserve existing snowflake validation", async () => {
  ...
  await expect(channels.fetchMessage("invalid", messageId)).rejects.toThrow(
    TypeError,
  );
  await expect(channels.fetchMessage(channelId, "invalid")).rejects.toThrow(
    TypeError,
  );
});
