...
test("REST retries idempotent server errors", async () => {
    ...
    globalThis.fetch = async () => {
        calls++;
        if (calls < 2) return new Response(JSON.stringify({ message: "busy" }), { status: 503 });
...
test("REST exposes rate-limit errors", async () => {
    const original = globalThis.fetch;
    globalThis.fetch = async () => new Response(JSON.stringify({ message: "slow down", code: 20028 }), { status: 429, headers: { "Retry-After": "0" } });
    try {
