...
describe("100% Comprehensive Codebase Coverage", () => {
    ...
    test("SlashCommandBuilder options, validations and subcommands", () => {
        ...
        expect(() => {
            ...
            for (let i = 0; i < 25; i++)
                g.addSubcommand((s) =>
                    s.setName(`sub${i}`).setDescription("d"),
                );
    ...
    test("Cache options, size getter, sweep, entries and dispose", async () => {
        ...
        const expCache = new Cache<string, number>({
            ttl: 5,
            sweepInterval: 1000,
        });
    ...
    test("Client Gateway event dispatchers coverage", async () => {
        ...
        gw.emit("GUILD_BAN_ADD", {
            guild_id: "200000000000000000",
            user: { id: "300000000000000000" },
        });
        gw.emit("GUILD_BAN_REMOVE", {
            guild_id: "200000000000000000",
            user: { id: "300000000000000000" },
        });
    ...
    test("Structures Base, User Avatar, Message and Interactions", async () => {
        ...
        const rawResponseWithData = new InteractionResponse(4, {
            content: "data",
        });
