...
test("message resource methods delegate through ChannelManager", async () => {
    ...
    expect(calls[1]).toEqual({ method: "POST", path: `/channels/${channelId}/messages`, body: { content: "again" } });
    expect(calls[2]).toEqual({ method: "POST", path: `/channels/${channelId}/messages`, body: { content: "reply" } });
    expect(calls[3]).toEqual({ method: "PATCH", path: `/channels/${channelId}/messages/${messageId}`, body: { content: "edited" } });
    expect(calls[4]).toEqual({ method: "DELETE", path: `/channels/${channelId}/messages/${messageId}` });
    expect(calls[5]).toEqual({ method: "POST", path: `/channels/${channelId}/messages/${messageId}/crosspost` });
});

test("detached structures fail clearly instead of bypassing REST", async () => {
    const { Channel, Message } = await import("../packages/structures/src/index.js");
    const detachedChannel = new Channel({ id: channelId, type: 0 });
    const detachedMessage = new Message(message);

    expect(() => detachedChannel.sendMessage({ content: "hello" })).toThrow("not attached to a client");
    expect(() => detachedMessage.reply("hello")).toThrow("not attached to a client");
});
