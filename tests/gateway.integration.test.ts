...
import { Gateway, GatewayError, GatewayOpcodes } from "../packages/ws/src/index.ts";
...
describe("Gateway integration lifecycle", () => {
    test("settles connect only after the socket opens", async () => {
        const gateway = new Gateway({ token: "token", intents: 1, reconnect: false });
        let settled = false;
        const promise = gateway.connect("wss://example.test").then(() => { settled = true; });
    ...
    test("responds to HELLO with IDENTIFY and schedules heartbeats", async () => {
        const gateway = new Gateway({ token: "token", intents: 513, reconnect: false });
        const promise = gateway.connect();
        const socket = FakeWebSocket.instances[0]!;
        socket.open();
        await promise;

        socket.receive({ op: GatewayOpcodes.Hello, d: { heartbeat_interval: 100 } });
    ...
    test("reconnects after a recoverable close", async () => {
        const gateway = new Gateway({ token: "token", intents: 1, reconnectBaseDelay: 1, reconnectMaxDelay: 1 });
    ...
    test("clears session state after INVALID_SESSION", async () => {
        ...
        socket.open();
        await promise;
        socket.receive({ op: GatewayOpcodes.Dispatch, t: "READY", s: 42, d: { session_id: "session", resume_gateway_url: "wss://resume.test" } });
    ...
    test("reports and closes a zombie connection", async () => {
        const gateway = new Gateway({ token: "token", intents: 1, reconnect: false, heartbeatAckTimeout: 10, zombieTimeout: 25 });
